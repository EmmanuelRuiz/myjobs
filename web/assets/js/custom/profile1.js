...
function buttons() {
	...
	$(".btn-delete-pub").unbind('click').click(function () {
		...
		var confirmacion = confirm('¿Estas seguro que quieres eliminar esta publicación?');
	...
	$(".btn-delete-comment").unbind('click').click(function () {
		...
		var confirmacion = confirm('¿Estas seguro que quieres eliminar este comentario?');

		if (confirmacion) {
			$(this).parent().parent().parent().parent().parent().addClass('hidden');

			$.ajax({
				url: URL + '/comment/remove/' + $(this).attr('data-id'),
				type: 'GET',
				success: function (response) {
					console.log(response);
				}
